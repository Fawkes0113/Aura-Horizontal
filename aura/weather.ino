#include <Arduino.h>
#include <WiFiManager.h>
#include <HTTPClient.h>
#include <ArduinoJson.h>
#include <time.h>
#include <lvgl.h>
#include <TFT_eSPI.h>
#include <XPT2046_Touchscreen.h>
#include <Preferences.h>
#include "esp_system.h"
#include "translations.h"

#define XPT2046_IRQ 36   // T_IRQ
#define XPT2046_MOSI 32  // T_DIN
#define XPT2046_MISO 39  // T_OUT
#define XPT2046_CLK 25   // T_CLK
#define XPT2046_CS 33    // T_CS
#define LCD_BACKLIGHT_PIN 21
#define SCREEN_WIDTH 320
#define SCREEN_HEIGHT 240
#define DRAW_BUF_SIZE (SCREEN_WIDTH * SCREEN_HEIGHT / 10 * (LV_COLOR_DEPTH / 8))

#define LATITUDE_DEFAULT "51.5074"
#define LONGITUDE_DEFAULT "-0.1278"
#define LOCATION_DEFAULT "London"
#define DEFAULT_CAPTIVE_SSID "Aura"
#define UPDATE_INTERVAL 600000UL  // 10 minutes

// Night mode starts at 10pm and ends at 6am
#define NIGHT_MODE_START_HOUR 22
#define NIGHT_MODE_END_HOUR 6

LV_FONT_DECLARE(lv_font_montserrat_latin_12);
LV_FONT_DECLARE(lv_font_montserrat_latin_14);
LV_FONT_DECLARE(lv_font_montserrat_latin_16);
LV_FONT_DECLARE(lv_font_montserrat_latin_20);
LV_FONT_DECLARE(lv_font_montserrat_latin_42);

// Global object for the display
TFT_eSPI tft = TFT_eSPI();

static Language current_language = LANG_EN;

// Font selection based on language
const lv_font_t* get_font_12() { return &lv_font_montserrat_latin_12; }
const lv_font_t* get_font_14() { return &lv_font_montserrat_latin_14; }
const lv_font_t* get_font_16() { return &lv_font_montserrat_latin_16; }
const lv_font_t* get_font_20() { return &lv_font_montserrat_latin_20; }
const lv_font_t* get_font_42() { return &lv_font_montserrat_latin_42; }

SPIClass touchscreenSPI = SPIClass(VSPI);
XPT2046_Touchscreen touchscreen(XPT2046_CS, XPT2046_IRQ);
uint32_t draw_buf[DRAW_BUF_SIZE / 4];
int x, y, z;

// Preferences
static Preferences prefs;
static bool use_fahrenheit = false;
static bool use_24_hour = false;
static bool use_night_mode = false;
static char latitude[16] = LATITUDE_DEFAULT;
static char longitude[16] = LONGITUDE_DEFAULT;
static String location = String(LOCATION_DEFAULT);
static char dd_opts[512];
static DynamicJsonDocument geoDoc(8 * 1024);
static JsonArray geoResults;

// Screen dimming variables
static bool night_mode_active = false;
static bool temp_screen_wakeup_active = false;
static lv_timer_t *temp_screen_wakeup_timer = nullptr;

// UI components
static lv_obj_t *lbl_today_temp;
static lv_obj_t *lbl_today_feels_like;
static lv_obj_t *img_today_icon;
static lv_obj_t *lbl_forecast;
static lv_obj_t *box_daily;
static lv_obj_t *box_hourly;
static lv_obj_t *lbl_daily_day[7];
static lv_obj_t *lbl_daily_high[7];
static lv_obj_t *lbl_daily_low[7];
static lv_obj_t *img_daily[7];
static lv_obj_t *lbl_hourly[7];
static lv_obj_t *lbl_precipitation_probability[7];
static lv_obj_t *lbl_hourly_temp[7];
static lv_obj_t *img_hourly[7];
static lv_obj_t *lbl_loc;
static lv_obj_t *loc_ta;
static lv_obj_t *results_dd;
static lv_obj_t *btn_close_loc;
static lv_obj_t *btn_close_obj;
static lv_obj_t *kb;
static lv_obj_t *settings_win;
static lv_obj_t *location_win = nullptr;
static lv_obj_t *unit_switch;
static lv_obj_t *clock_24hr_switch;
static lv_obj_t *night_mode_switch;
static lv_obj_t *language_dropdown;
static lv_obj_t *lbl_clock;

// Weather icons & Images (declarations must be present)
LV_IMG_DECLARE(icon_blizzard); LV_IMG_DECLARE(image_blizzard);
LV_IMG_DECLARE(icon_blowing_snow); LV_IMG_DECLARE(image_blowing_snow);
LV_IMG_DECLARE(icon_clear_night); LV_IMG_DECLARE(image_clear_night);
LV_IMG_DECLARE(icon_cloudy); LV_IMG_DECLARE(image_cloudy);
LV_IMG_DECLARE(icon_drizzle); LV_IMG_DECLARE(image_drizzle);
LV_IMG_DECLARE(icon_flurries); LV_IMG_DECLARE(image_flurries);
LV_IMG_DECLARE(icon_haze_fog_dust_smoke); LV_IMG_DECLARE(image_haze_fog_dust_smoke);
LV_IMG_DECLARE(icon_heavy_rain); LV_IMG_DECLARE(image_heavy_rain);
LV_IMG_DECLARE(icon_heavy_snow); LV_IMG_DECLARE(image_heavy_snow);
LV_IMG_DECLARE(icon_isolated_scattered_tstorms_day); LV_IMG_DECLARE(image_isolated_scattered_tstorms_day);
LV_IMG_DECLARE(icon_isolated_scattered_tstorms_night); LV_IMG_DECLARE(image_isolated_scattered_tstorms_night);
LV_IMG_DECLARE(icon_mostly_clear_night); LV_IMG_DECLARE(image_mostly_clear_night);
LV_IMG_DECLARE(icon_mostly_cloudy_day); LV_IMG_DECLARE(image_mostly_cloudy_day);
LV_IMG_DECLARE(icon_mostly_cloudy_night); LV_IMG_DECLARE(image_mostly_cloudy_night);
LV_IMG_DECLARE(icon_mostly_sunny); LV_IMG_DECLARE(image_mostly_sunny);
LV_IMG_DECLARE(icon_partly_cloudy); LV_IMG_DECLARE(image_partly_cloudy);
LV_IMG_DECLARE(icon_partly_cloudy_night); LV_IMG_DECLARE(image_partly_cloudy_night);
LV_IMG_DECLARE(icon_scattered_showers_day); LV_IMG_DECLARE(image_scattered_showers_day);
LV_IMG_DECLARE(icon_scattered_showers_night); LV_IMG_DECLARE(image_scattered_showers_night);
LV_IMG_DECLARE(icon_showers_rain); LV_IMG_DECLARE(image_showers_rain);
LV_IMG_DECLARE(icon_sleet_hail); LV_IMG_DECLARE(image_sleet_hail);
LV_IMG_DECLARE(icon_snow_showers_snow); LV_IMG_DECLARE(image_snow_showers_snow);
LV_IMG_DECLARE(icon_strong_tstorms); LV_IMG_DECLARE(image_strong_tstorms);
LV_IMG_DECLARE(icon_sunny); LV_IMG_DECLARE(image_sunny);
LV_IMG_DECLARE(icon_tornado); LV_IMG_DECLARE(image_tornado);
LV_IMG_DECLARE(icon_wintry_mix_rain_snow); LV_IMG_DECLARE(image_wintry_mix_rain_snow);

// Forward declarations
void create_ui();
void fetch_and_update_weather();
void create_settings_window();
static void screen_event_cb(lv_event_t *e);
static void settings_event_handler(lv_event_t *e);
const lv_img_dsc_t *choose_image(int wmo_code, int is_day);
const lv_img_dsc_t *choose_icon(int wmo_code, int is_day);
bool night_mode_should_be_active();
void activate_night_mode();
void deactivate_night_mode();
void check_for_night_mode();
void handle_temp_screen_wakeup_timeout(lv_timer_t *timer);
void daily_cb(lv_event_t *e);
void hourly_cb(lv_event_t *e);
void do_geocode_query(const char *q);
static void reset_confirm_yes_cb(lv_event_t *e);
static void reset_confirm_no_cb(lv_event_t *e);
void wifi_splash_screen();
void apModeCallback(WiFiManager *mgr);

int day_of_week(int y, int m, int d) {
  static const int t[] = { 0, 3, 2, 5, 0, 3, 5, 1, 4, 6, 2, 4 };
  if (m < 3) y -= 1;
  return (y + y / 4 - y / 100 + y / 400 + t[m - 1] + d) % 7;
}

String hour_of_day(int hour) {
  const LocalizedStrings* strings = get_strings(current_language);
  if(hour < 0 || hour > 23) return String(strings->invalid_hour);

  if (use_24_hour) {
    if (hour < 10) return String("0") + String(hour);
    else return String(hour);
  } else {
    if(hour == 0) return String("12") + strings->am;
    if(hour == 12) return String("12") + strings->pm;
    int displayHour = hour % 12;
    return String(displayHour) + (hour < 12 ? strings->am : strings->pm);
  }
}

String urlencode(const String &str) {
  String encoded = "";
  char buf[5];
  for (size_t i = 0; i < str.length(); i++) {
    char c = str.charAt(i);
    if (isalnum(c) || c == '-' || c == '_' || c == '.' || c == '~') {
      encoded += c;
    } else {
      sprintf(buf, "%%%02X", (unsigned char)c);
      encoded += buf;
    }
  }
  return encoded;
}

static void update_clock(lv_timer_t *timer) {
  struct tm timeinfo;
  check_for_night_mode();
  if (!getLocalTime(&timeinfo)) return;

  const LocalizedStrings* strings = get_strings(current_language);
  char buf[16];
  if (use_24_hour) {
    snprintf(buf, sizeof(buf), "%02d:%02d", timeinfo.tm_hour, timeinfo.tm_min);
  } else {
    int hour = timeinfo.tm_hour % 12;
    if(hour == 0) hour = 12;
    snprintf(buf, sizeof(buf), "%d:%02d%s", hour, timeinfo.tm_min, (timeinfo.tm_hour < 12) ? strings->am : strings->pm);
  }
  lv_label_set_text(lbl_clock, buf);
}

static void ta_event_cb(lv_event_t *e) {
  lv_obj_t *ta = lv_event_get_target(e);
  lv_obj_t *kb = (lv_obj_t *)lv_event_get_user_data(e);
  lv_keyboard_set_textarea(kb, ta);
  lv_obj_move_foreground(kb);
  lv_obj_clear_flag(kb, LV_OBJ_FLAG_HIDDEN);
}

static void kb_event_cb(lv_event_t *e) {
  lv_obj_t *kb = lv_event_get_target(e);
  lv_obj_add_flag(kb, LV_OBJ_FLAG_HIDDEN);

  if (lv_event_get_code(e) == LV_EVENT_READY) {
    const char *loc = lv_textarea_get_text(loc_ta);
    if (strlen(loc) > 0) {
      do_geocode_query(loc);
    }
  }
}

static void ta_defocus_cb(lv_event_t *e) {
  lv_obj_add_flag((lv_obj_t *)lv_event_get_user_data(e), LV_OBJ_FLAG_HIDDEN);
}

void touchscreen_read(lv_indev_t *indev, lv_indev_data_t *data) {
  if (touchscreen.tirqTouched() && touchscreen.touched()) {
    TS_Point p = touchscreen.getPoint();
    
    // Remapped for rotation 3
    x = map(p.y, 200, 3700, 1, SCREEN_WIDTH); 
    y = map(p.x, 200, 3800, SCREEN_HEIGHT, 1);
    
    data->state = LV_INDEV_STATE_PRESSED;
    data->point.x = x;
    data->point.y = y;
  } else {
    data->state = LV_INDEV_STATE_RELEASED;
  }
}

// ====================================================================
// ========================= SETUP FUNCTION ===========================
// ====================================================================
void setup() {
  Serial.begin(115200);
  delay(100);

  // --- Start LVGL and TFT/Touch Drivers ---
  lv_init();
  
  tft.init(); // Initialize TFT
  pinMode(LCD_BACKLIGHT_PIN, OUTPUT);

  // Initialize Touchscreen
  touchscreenSPI.begin(XPT2046_CLK, XPT2046_MISO, XPT2046_MOSI, XPT2046_CS);
  touchscreen.begin(touchscreenSPI);

  // --- CREATE THE LVGL DISPLAY DRIVER ---
  // This step links LVGL to the tft object
  lv_display_t *disp = lv_tft_espi_create(SCREEN_WIDTH, SCREEN_HEIGHT, draw_buf, sizeof(draw_buf));

  // --- SET ROTATION (CRITICAL STEP) ---
  // Set rotation AFTER the display driver is created
  tft.setRotation(3);
  touchscreen.setRotation(3);

  // --- Create LVGL Input Driver ---
  lv_indev_t *indev = lv_indev_create();
  lv_indev_set_type(indev, LV_INDEV_TYPE_POINTER);
  lv_indev_set_read_cb(indev, touchscreen_read);

  // --- Load Preferences ---
  prefs.begin("weather", false);
  String lat = prefs.getString("latitude", LATITUDE_DEFAULT);
  lat.toCharArray(latitude, sizeof(latitude));
  String lon = prefs.getString("longitude", LONGITUDE_DEFAULT);
  lon.toCharArray(longitude, sizeof(longitude));
  use_fahrenheit = prefs.getBool("useFahrenheit", false);
  location = prefs.getString("location", LOCATION_DEFAULT);
  use_night_mode = prefs.getBool("useNightMode", false);
  uint32_t brightness = prefs.getUInt("brightness", 255);
  use_24_hour = prefs.getBool("use24Hour", false);
  current_language = (Language)prefs.getUInt("language", LANG_EN);
  analogWrite(LCD_BACKLIGHT_PIN, brightness);

  // --- Network and UI Setup ---
  WiFiManager wm;
  wm.setAPCallback(apModeCallback);
  wm.autoConnect(DEFAULT_CAPTIVE_SSID);

  lv_timer_create(update_clock, 1000, NULL);

  lv_obj_clean(lv_scr_act());
  create_ui();
  fetch_and_update_weather();
}

void flush_wifi_splashscreen(uint32_t ms = 200) {
  uint32_t start = millis();
  while (millis() - start < ms) {
    lv_timer_handler();
    delay(5);
  }
}

void apModeCallback(WiFiManager *mgr) {
  wifi_splash_screen();
  flush_wifi_splashscreen();
}

void loop() {
  lv_timer_handler();
  static uint32_t last = millis();

  if (millis() - last >= UPDATE_INTERVAL) {
    fetch_and_update_weather();
    last = millis();
  }

  delay(5);
}

void wifi_splash_screen() {
  lv_obj_t *scr = lv_scr_act();
  lv_obj_clean(scr);
  lv_obj_set_style_bg_color(scr, lv_color_hex(0x4c8cb9), LV_PART_MAIN);
  const LocalizedStrings* strings = get_strings(current_language);
  lv_obj_t *lbl = lv_label_create(scr);
  lv_label_set_text(lbl, strings->wifi_config);
  lv_obj_set_style_text_font(lbl, get_font_14(), 0);
  lv_obj_center(lbl);
  lv_scr_load(scr);
}

void create_ui() {
  lv_obj_t *scr = lv_scr_act();
  lv_obj_set_style_bg_color(scr, lv_color_hex(0x4c8cb9), LV_PART_MAIN);
  lv_obj_set_style_bg_grad_color(scr, lv_color_hex(0xa6cdec), LV_PART_MAIN);
  lv_obj_set_style_bg_grad_dir(scr, LV_GRAD_DIR_VER, LV_PART_MAIN);

  lv_obj_add_event_cb(scr, screen_event_cb, LV_EVENT_CLICKED, NULL);

  static lv_style_t default_label_style;
  lv_style_init(&default_label_style);
  lv_style_set_text_color(&default_label_style, lv_color_white());

  const LocalizedStrings* strings = get_strings(current_language);

  // --- Left Side: Current Weather ---
  img_today_icon = lv_img_create(scr);
  lv_img_set_src(img_today_icon, &image_partly_cloudy);
  lv_obj_align(img_today_icon, LV_ALIGN_LEFT_MID, 5, -40);

  lbl_today_temp = lv_label_create(scr);
  lv_label_set_text(lbl_today_temp, strings->temp_placeholder);
  lv_obj_add_style(lbl_today_temp, &default_label_style, 0);
  lv_obj_set_style_text_font(lbl_today_temp, get_font_42(), 0);
  lv_obj_align(lbl_today_temp, LV_ALIGN_LEFT_MID, 15, 30);

  lbl_today_feels_like = lv_label_create(scr);
  lv_label_set_text(lbl_today_feels_like, strings->feels_like_temp);
  lv_obj_add_style(lbl_today_feels_like, &default_label_style, 0);
  lv_obj_set_style_text_font(lbl_today_feels_like, get_font_14(), 0);
  lv_obj_align(lbl_today_feels_like, LV_ALIGN_LEFT_MID, 15, 65);

  // --- Right Side: Forecast ---
  lbl_forecast = lv_label_create(scr);
  lv_label_set_text(lbl_forecast, strings->seven_day_forecast);
  lv_obj_add_style(lbl_forecast, &default_label_style, 0);
  lv_obj_set_style_text_font(lbl_forecast, get_font_12(), 0);
  lv_obj_align(lbl_forecast, LV_ALIGN_TOP_RIGHT, -15, 5);
  
  // Daily Box
  box_daily = lv_obj_create(scr);
  lv_obj_set_size(box_daily, 165, 205);
  lv_obj_align(box_daily, LV_ALIGN_TOP_RIGHT, -5, 25);
  lv_obj_set_style_bg_color(box_daily, lv_color_hex(0x5e9bc8), 0);
  lv_obj_set_style_border_width(box_daily, 0, 0);
  lv_obj_set_style_pad_all(box_daily, 5, 0);
  lv_obj_clear_flag(box_daily, LV_OBJ_FLAG_SCROLLABLE);
  lv_obj_add_event_cb(box_daily, daily_cb, LV_EVENT_CLICKED, NULL);

  for (int i = 0; i < 7; i++) {
    int y_pos = i * 28;
    lbl_daily_day[i] = lv_label_create(box_daily);
    lv_obj_add_style(lbl_daily_day[i], &default_label_style, 0);
    lv_obj_set_style_text_font(lbl_daily_day[i], get_font_16(), 0);
    lv_obj_align(lbl_daily_day[i], LV_ALIGN_TOP_LEFT, 2, y_pos);

    img_daily[i] = lv_img_create(box_daily);
    lv_img_set_src(img_daily[i], &icon_partly_cloudy);
    lv_obj_align(img_daily[i], LV_ALIGN_TOP_LEFT, 55, y_pos);
    
    lbl_daily_high[i] = lv_label_create(box_daily);
    lv_obj_add_style(lbl_daily_high[i], &default_label_style, 0);
    lv_obj_set_style_text_font(lbl_daily_high[i], get_font_16(), 0);
    lv_obj_align(lbl_daily_high[i], LV_ALIGN_TOP_RIGHT, 0, y_pos);

    lbl_daily_low[i] = lv_label_create(box_daily);
    lv_obj_set_style_text_color(lbl_daily_low[i], lv_color_hex(0xb9ecff), 0);
    lv_obj_set_style_text_font(lbl_daily_low[i], get_font_16(), 0);
    lv_obj_align(lbl_daily_low[i], LV_ALIGN_TOP_RIGHT, -45, y_pos);
  }

  // Hourly Box (same position, initially hidden)
  box_hourly = lv_obj_create(scr);
  lv_obj_set_size(box_hourly, 165, 205);
  lv_obj_align(box_hourly, LV_ALIGN_TOP_RIGHT, -5, 25);
  lv_obj_set_style_bg_color(box_hourly, lv_color_hex(0x5e9bc8), 0);
  lv_obj_set_style_border_width(box_hourly, 0, 0);
  lv_obj_set_style_pad_all(box_hourly, 5, 0);
  lv_obj_clear_flag(box_hourly, LV_OBJ_FLAG_SCROLLABLE);
  lv_obj_add_event_cb(box_hourly, hourly_cb, LV_EVENT_CLICKED, NULL);

  for (int i = 0; i < 7; i++) {
    int y_pos = i * 28;
    lbl_hourly[i] = lv_label_create(box_hourly);
    lv_obj_add_style(lbl_hourly[i], &default_label_style, 0);
    lv_obj_set_style_text_font(lbl_hourly[i], get_font_16(), 0);
    lv_obj_align(lbl_hourly[i], LV_ALIGN_TOP_LEFT, 2, y_pos);

    img_hourly[i] = lv_img_create(box_hourly);
    lv_img_set_src(img_hourly[i], &icon_partly_cloudy);
    lv_obj_align(img_hourly[i], LV_ALIGN_TOP_LEFT, 55, y_pos);

    lbl_hourly_temp[i] = lv_label_create(box_hourly);
    lv_obj_add_style(lbl_hourly_temp[i], &default_label_style, 0);
    lv_obj_set_style_text_font(lbl_hourly_temp[i], get_font_16(), 0);
    lv_obj_align(lbl_hourly_temp[i], LV_ALIGN_TOP_RIGHT, 0, y_pos);
    
    lbl_precipitation_probability[i] = lv_label_create(box_hourly);
    lv_obj_set_style_text_color(lbl_precipitation_probability[i], lv_color_hex(0xb9ecff), 0);
    lv_obj_set_style_text_font(lbl_precipitation_probability[i], get_font_16(), 0);
    lv_obj_align(lbl_precipitation_probability[i], LV_ALIGN_TOP_RIGHT, -45, y_pos);
  }
  lv_obj_add_flag(box_hourly, LV_OBJ_FLAG_HIDDEN);

  // Clock
  lbl_clock = lv_label_create(scr);
  lv_obj_set_style_text_color(lbl_clock, lv_color_hex(0xb9ecff), 0);
  lv_obj_set_style_text_font(lbl_clock, get_font_14(), 0);
  lv_label_set_text(lbl_clock, "");
  lv_obj_align(lbl_clock, LV_ALIGN_TOP_LEFT, 10, 2);
}

// ... (rest of the functions: populate_results_dropdown, location_save_event_cb, etc.)
// ... (No changes needed in the functions below this line)

void populate_results_dropdown() {
  dd_opts[0] = '\0';
  for (JsonObject item : geoResults) {
    strcat(dd_opts, item["name"].as<const char *>());
    if (item["admin1"]) {
      strcat(dd_opts, ", ");
      strcat(dd_opts, item["admin1"].as<const char *>());
    }
    strcat(dd_opts, "\n");
  }

  if (geoResults.size() > 0) {
    lv_dropdown_set_options_static(results_dd, dd_opts);
    lv_obj_add_flag(results_dd, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_set_style_bg_color(btn_close_loc, lv_palette_main(LV_PALETTE_GREEN), 0);
    lv_obj_add_flag(btn_close_loc, LV_OBJ_FLAG_CLICKABLE);
  }
}

static void location_save_event_cb(lv_event_t *e) {
  JsonArray *pres = (JsonArray*)lv_event_get_user_data(e);
  uint16_t idx = lv_dropdown_get_selected(results_dd);

  JsonObject obj = (*pres)[idx];
  snprintf(latitude, sizeof(latitude), "%.6f", obj["latitude"].as<double>());
  snprintf(longitude, sizeof(longitude), "%.6f", obj["longitude"].as<double>());
  prefs.putString("latitude", latitude);
  prefs.putString("longitude", longitude);

  String new_loc = obj["name"].as<String>();
  if (obj["admin1"]) {
    new_loc += ", ";
    new_loc += obj["admin1"].as<String>();
  }
  prefs.putString("location", new_loc);
  location = new_loc;
  
  fetch_and_update_weather();
  lv_obj_del(location_win);
  location_win = nullptr;
}

static void location_cancel_event_cb(lv_event_t *e) {
  lv_obj_del(location_win);
  location_win = nullptr;
}

void screen_event_cb(lv_event_t *e) { create_settings_window(); }

void daily_cb(lv_event_t *e) {
  const LocalizedStrings* strings = get_strings(current_language);
  lv_obj_add_flag(box_daily, LV_OBJ_FLAG_HIDDEN);
  lv_label_set_text(lbl_forecast, strings->hourly_forecast);
  lv_obj_clear_flag(box_hourly, LV_OBJ_FLAG_HIDDEN);
}

void hourly_cb(lv_event_t *e) {
  const LocalizedStrings* strings = get_strings(current_language);
  lv_obj_add_flag(box_hourly, LV_OBJ_FLAG_HIDDEN);
  lv_label_set_text(lbl_forecast, strings->seven_day_forecast);
  lv_obj_clear_flag(box_daily, LV_OBJ_FLAG_HIDDEN);
}

static void reset_wifi_event_handler(lv_event_t *e) {
  const LocalizedStrings* strings = get_strings(current_language);
  lv_obj_t *mbox = lv_msgbox_create(lv_scr_act(), strings->reset, strings->reset_confirmation, NULL, true);
  lv_obj_t *btn_yes = lv_msgbox_add_footer_button(mbox, strings->reset);
  lv_msgbox_add_footer_button(mbox, strings->cancel);
  lv_obj_set_width(mbox, 280);
  lv_obj_center(mbox);
  lv_obj_add_event_cb(btn_yes, reset_confirm_yes_cb, LV_EVENT_CLICKED, NULL);
}

static void reset_confirm_yes_cb(lv_event_t *e) {
  Serial.println("Clearing Wi-Fi creds and rebooting");
  WiFiManager wm;
  wm.resetSettings();
  delay(100);
  esp_restart();
}

void create_location_dialog();

static void change_location_event_cb(lv_event_t *e) {
  if (location_win) return;
  create_location_dialog();
}

void create_location_dialog() {
  const LocalizedStrings* strings = get_strings(current_language);
  location_win = lv_win_create(lv_scr_act());
  lv_win_add_title(location_win, strings->change_location);
  lv_obj_set_size(location_win, 320, 240);
  lv_obj_center(location_win);

  lv_obj_t *cont = lv_win_get_content(location_win);

  lv_obj_t *lbl = lv_label_create(cont);
  lv_label_set_text(lbl, strings->city);
  lv_obj_align(lbl, LV_ALIGN_TOP_LEFT, 5, 10);

  loc_ta = lv_textarea_create(cont);
  lv_textarea_set_one_line(loc_ta, true);
  lv_textarea_set_placeholder_text(loc_ta, strings->city_placeholder);
  lv_obj_set_width(loc_ta, 240);
  lv_obj_align_to(loc_ta, lbl, LV_ALIGN_OUT_RIGHT_MID, 5, 0);

  lv_obj_add_event_cb(loc_ta, ta_event_cb, LV_EVENT_CLICKED, kb);
  lv_obj_add_event_cb(loc_ta, ta_defocus_cb, LV_EVENT_DEFOCUSED, kb);

  results_dd = lv_dropdown_create(cont);
  lv_obj_set_width(results_dd, 280);
  lv_obj_align(results_dd, LV_ALIGN_TOP_LEFT, 5, 40);
  lv_dropdown_set_options(results_dd, "");
  lv_obj_clear_flag(results_dd, LV_OBJ_FLAG_CLICKABLE);

  btn_close_loc = lv_btn_create(cont);
  lv_obj_set_size(btn_close_loc, 100, 40);
  lv_obj_align(btn_close_loc, LV_ALIGN_BOTTOM_RIGHT, -5, -5);
  lv_obj_add_event_cb(btn_close_loc, location_save_event_cb, LV_EVENT_CLICKED, &geoResults);
  lv_obj_set_style_bg_color(btn_close_loc, lv_palette_main(LV_PALETTE_GREY), 0);
  lv_obj_clear_flag(btn_close_loc, LV_OBJ_FLAG_CLICKABLE);
  lv_obj_t *lbl_close = lv_label_create(btn_close_loc);
  lv_label_set_text(lbl_close, strings->save);
  lv_obj_center(lbl_close);

  lv_obj_t *btn_cancel_loc = lv_btn_create(cont);
  lv_obj_set_size(btn_cancel_loc, 100, 40);
  lv_obj_align_to(btn_cancel_loc, btn_close_loc, LV_ALIGN_OUT_LEFT_MID, -10, 0);
  lv_obj_add_event_cb(btn_cancel_loc, location_cancel_event_cb, LV_EVENT_CLICKED, NULL);
  lv_obj_t *lbl_cancel = lv_label_create(btn_cancel_loc);
  lv_label_set_text(lbl_cancel, strings->cancel);
  lv_obj_center(lbl_cancel);
}

void create_settings_window() {
  if (settings_win) return;
  const LocalizedStrings* strings = get_strings(current_language);
  settings_win = lv_win_create(lv_scr_act());
  lv_win_add_title(settings_win, strings->aura_settings);
  lv_obj_set_size(settings_win, 320, 240);
  lv_obj_center(settings_win);

  lv_obj_t *cont = lv_win_get_content(settings_win);

  // Layout container
  lv_obj_t * layout = lv_obj_create(cont);
  lv_obj_set_size(layout, LV_PCT(100), LV_PCT(100));
  lv_obj_set_flex_flow(layout, LV_FLEX_FLOW_COLUMN);
  lv_obj_set_flex_align(layout, LV_FLEX_ALIGN_SPACE_EVENLY, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_CENTER);
  lv_obj_set_style_pad_all(layout, 5, 0);

  // Brightness Row
  lv_obj_t* row1 = lv_obj_create(layout);
  lv_obj_set_width(row1, LV_PCT(100));
  lv_obj_set_flex_flow(row1, LV_FLEX_FLOW_ROW);
  lv_obj_set_flex_align(row1, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
  lv_obj_t *lbl_b = lv_label_create(row1);
  lv_label_set_text(lbl_b, strings->brightness);
  lv_obj_t *slider = lv_slider_create(row1);
  lv_obj_set_flex_grow(slider, 1);
  lv_slider_set_range(slider, 1, 255);
  lv_slider_set_value(slider, prefs.getUInt("brightness", 128), LV_ANIM_OFF);
  lv_obj_add_event_cb(slider, [](lv_event_t *e){
    analogWrite(LCD_BACKLIGHT_PIN, lv_slider_get_value(lv_event_get_target(e)));
    prefs.putUInt("brightness", lv_slider_get_value(lv_event_get_target(e)));
  }, LV_EVENT_VALUE_CHANGED, NULL);

  // Switches Row
  lv_obj_t* row2 = lv_obj_create(layout);
  lv_obj_set_width(row2, LV_PCT(100));
  lv_obj_set_flex_flow(row2, LV_FLEX_FLOW_ROW);
  lv_obj_set_flex_align(row2, LV_FLEX_ALIGN_SPACE_BETWEEN, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
  // Night Mode
  lv_obj_t *lbl_nm = lv_label_create(row2); lv_label_set_text(lbl_nm, strings->use_night_mode);
  night_mode_switch = lv_switch_create(row2); if(use_night_mode) lv_obj_add_state(night_mode_switch, LV_STATE_CHECKED);
  // Fahrenheit
  lv_obj_t *lbl_u = lv_label_create(row2); lv_label_set_text(lbl_u, strings->use_fahrenheit);
  unit_switch = lv_switch_create(row2); if(use_fahrenheit) lv_obj_add_state(unit_switch, LV_STATE_CHECKED);
  // 24 Hour
  lv_obj_t *lbl_24hr = lv_label_create(row2); lv_label_set_text(lbl_24hr, strings->use_24hr);
  clock_24hr_switch = lv_switch_create(row2); if(use_24_hour) lv_obj_add_state(clock_24hr_switch, LV_STATE_CHECKED);
  
  lv_obj_add_event_cb(night_mode_switch, settings_event_handler, LV_EVENT_VALUE_CHANGED, NULL);
  lv_obj_add_event_cb(unit_switch, settings_event_handler, LV_EVENT_VALUE_CHANGED, NULL);
  lv_obj_add_event_cb(clock_24hr_switch, settings_event_handler, LV_EVENT_VALUE_CHANGED, NULL);

  // Language & Location Row
  lv_obj_t* row3 = lv_obj_create(layout);
  lv_obj_set_width(row3, LV_PCT(100));
  lv_obj_set_flex_flow(row3, LV_FLEX_FLOW_ROW);
  lv_obj_set_flex_align(row3, LV_FLEX_ALIGN_SPACE_BETWEEN, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
  lv_obj_t *lbl_lang = lv_label_create(row3); lv_label_set_text(lbl_lang, strings->language_label);
  language_dropdown = lv_dropdown_create(row3);
  lv_dropdown_set_options(language_dropdown, "English\nEspañol\nDeutsch\nFrançais\nTürkçe\nSvenska\nItaliano");
  lv_dropdown_set_selected(language_dropdown, current_language);
  lv_obj_add_event_cb(language_dropdown, settings_event_handler, LV_EVENT_VALUE_CHANGED, NULL);
  
  lv_obj_t *lbl_loc_l = lv_label_create(row3); lv_label_set_text(lbl_loc_l, strings->location);
  lbl_loc = lv_label_create(row3); lv_label_set_text(lbl_loc, location.c_str());

  // Buttons Row
  lv_obj_t* row4 = lv_obj_create(layout);
  lv_obj_set_width(row4, LV_PCT(100));
  lv_obj_set_flex_flow(row4, LV_FLEX_FLOW_ROW);
  lv_obj_set_flex_align(row4, LV_FLEX_ALIGN_SPACE_EVENLY, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);

  lv_obj_t *btn_change_loc = lv_btn_create(row4); lv_obj_add_event_cb(btn_change_loc, change_location_event_cb, LV_EVENT_CLICKED, NULL);
  lv_obj_t *lbl_chg = lv_label_create(btn_change_loc); lv_label_set_text(lbl_chg, strings->location_btn); lv_obj_center(lbl_chg);
  
  lv_obj_t *btn_reset = lv_btn_create(row4); lv_obj_add_event_cb(btn_reset, reset_wifi_event_handler, LV_EVENT_CLICKED, nullptr);
  lv_obj_set_style_bg_color(btn_reset, lv_palette_main(LV_PALETTE_RED), 0);
  lv_obj_t *lbl_reset = lv_label_create(btn_reset); lv_label_set_text(lbl_reset, strings->reset_wifi); lv_obj_center(lbl_reset);

  if (!kb) {
    kb = lv_keyboard_create(lv_scr_act());
    lv_obj_add_flag(kb, LV_OBJ_FLAG_HIDDEN);
    lv_obj_add_event_cb(kb, kb_event_cb, LV_EVENT_ALL, NULL);
  }

  btn_close_obj = lv_win_add_btn(settings_win, LV_SYMBOL_CLOSE, 40);
  lv_obj_add_event_cb(btn_close_obj, settings_event_handler, LV_EVENT_CLICKED, NULL);
}

static void settings_event_handler(lv_event_t *e) {
  lv_obj_t *tgt = lv_event_get_target(e);
  lv_event_code_t code = lv_event_get_code(e);

  if (code == LV_EVENT_VALUE_CHANGED) {
    if (tgt == unit_switch) use_fahrenheit = lv_obj_has_state(tgt, LV_STATE_CHECKED);
    if (tgt == clock_24hr_switch) use_24_hour = lv_obj_has_state(tgt, LV_STATE_CHECKED);
    if (tgt == night_mode_switch) use_night_mode = lv_obj_has_state(tgt, LV_STATE_CHECKED);
    if (tgt == language_dropdown) {
        current_language = (Language)lv_dropdown_get_selected(tgt);
        // Language change requires full UI rebuild
        prefs.putUInt("language", current_language);
        lv_obj_del(settings_win);
        settings_win = nullptr;
        lv_obj_clean(lv_scr_act());
        create_ui();
        fetch_and_update_weather();
        return;
    }
  }

  if (code == LV_EVENT_CLICKED && tgt == btn_close_obj) {
    prefs.putBool("useFahrenheit", use_fahrenheit);
    prefs.putBool("use24Hour", use_24_hour);
    prefs.putBool("useNightMode", use_night_mode);
    lv_obj_del(settings_win);
    settings_win = nullptr;
    fetch_and_update_weather();
  }
}

void fetch_and_update_weather() {
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("WiFi not connected. Re-checking...");
    return;
  }

  String url = String("http://api.open-meteo.com/v1/forecast?latitude=")
               + latitude + "&longitude=" + longitude
               + "&current=temperature_2m,apparent_temperature,is_day,weather_code"
               + "&daily=temperature_2m_min,temperature_2m_max,weather_code"
               + "&hourly=temperature_2m,precipitation_probability,is_day,weather_code"
               + "&forecast_hours=7&timezone=auto";

  HTTPClient http;
  http.begin(url);
  int httpCode = http.GET();
  if (httpCode == HTTP_CODE_OK) {
    DynamicJsonDocument doc(32 * 1024);
    if (deserializeJson(doc, http.getStream()) == DeserializationError::Ok) {
      float t_now = doc["current"]["temperature_2m"];
      float t_ap = doc["current"]["apparent_temperature"];
      if (use_fahrenheit) {
        t_now = t_now * 1.8 + 32;
        t_ap = t_ap * 1.8 + 32;
      }
      const LocalizedStrings* s = get_strings(current_language);
      char unit = use_fahrenheit ? 'F' : 'C';

      configTime(doc["utc_offset_seconds"], 0, "pool.ntp.org");
      
      lv_label_set_text_fmt(lbl_today_temp, "%.0f°%c", t_now, unit);
      lv_label_set_text_fmt(lbl_today_feels_like, "%s %.0f°%c", s->feels_like_temp, t_ap, unit);
      lv_img_set_src(img_today_icon, choose_image(doc["current"]["weather_code"], doc["current"]["is_day"]));

      JsonArray times = doc["daily"]["time"];
      for (int i = 0; i < 7; i++) {
        const char *date = times[i];
        int dow = day_of_week(atoi(date), atoi(date + 5), atoi(date + 8));
        const char *dayStr = (i == 0) ? s->today : s->weekdays[dow];
        
        float mn = doc["daily"]["temperature_2m_min"][i];
        float mx = doc["daily"]["temperature_2m_max"][i];
        if (use_fahrenheit) { mn = mn * 1.8 + 32; mx = mx * 1.8 + 32; }

        lv_label_set_text(lbl_daily_day[i], dayStr);
        lv_label_set_text_fmt(lbl_daily_high[i], "%.0f°", mx);
        lv_label_set_text_fmt(lbl_daily_low[i], "%.0f°", mn);
        lv_img_set_src(img_daily[i], choose_icon(doc["daily"]["weather_code"][i], 1));
      }

      JsonArray hourly_times = doc["hourly"]["time"];
      for (int i = 0; i < 7; i++) {
        float temp = doc["hourly"]["temperature_2m"][i];
        if (use_fahrenheit) { temp = temp * 1.8 + 32; }

        lv_label_set_text(lbl_hourly[i], (i == 0) ? s->now : hour_of_day(atoi(hourly_times[i].as<const char *>() + 11)).c_str());
        lv_label_set_text_fmt(lbl_precipitation_probability[i], "%.0f%%", doc["hourly"]["precipitation_probability"][i].as<float>());
        lv_label_set_text_fmt(lbl_hourly_temp[i], "%.0f°%c", temp, unit);
        lv_img_set_src(img_hourly[i], choose_icon(doc["hourly"]["weather_code"][i], doc["hourly"]["is_day"][i]));
      }
    }
  }
  http.end();
}

// ... (choose_image and choose_icon functions are unchanged)
const lv_img_dsc_t* choose_image(int code, int is_day) {
  switch (code) {
    case  0: return is_day ? &image_sunny : &image_clear_night;
    case  1: return is_day ? &image_mostly_sunny : &image_mostly_clear_night;
    case  2: return is_day ? &image_partly_cloudy : &image_partly_cloudy_night;
    case  3: return &image_cloudy;
    case 45: case 48: return &image_haze_fog_dust_smoke;
    case 51: case 53: case 55: return &image_drizzle;
    case 56: case 57: return &image_sleet_hail;
    case 61: return is_day ? &image_scattered_showers_day : &image_scattered_showers_night;
    case 63: return &image_showers_rain;
    case 65: return &image_heavy_rain;
    case 66: case 67: return &image_wintry_mix_rain_snow;
    case 71: case 73: case 75: case 85: return &image_snow_showers_snow;
    case 77: return &image_flurries;
    case 80: case 81: return is_day ? &image_scattered_showers_day : &image_scattered_showers_night;
    case 82: return &image_heavy_rain;
    case 86: return &image_heavy_snow;
    case 95: return is_day ? &image_isolated_scattered_tstorms_day : &image_isolated_scattered_tstorms_night;
    case 96: case 99: return &image_strong_tstorms;
    default: return is_day ? &image_mostly_cloudy_day : &image_mostly_cloudy_night;
  }
}

const lv_img_dsc_t* choose_icon(int code, int is_day) {
  switch (code) {
    case  0: return is_day ? &icon_sunny : &icon_clear_night;
    case  1: return is_day ? &icon_mostly_sunny : &icon_mostly_clear_night;
    case  2: return is_day ? &icon_partly_cloudy : &icon_partly_cloudy_night;
    case  3: return &icon_cloudy;
    case 45: case 48: return &icon_haze_fog_dust_smoke;
    case 51: case 53: case 55: return &icon_drizzle;
    case 56: case 57: return &icon_sleet_hail;
    case 61: return is_day ? &icon_scattered_showers_day : &icon_scattered_showers_night;
    case 63: return &icon_showers_rain;
    case 65: return &icon_heavy_rain;
    case 66: case 67: return &icon_wintry_mix_rain_snow;
    case 71: case 73: case 75: case 85: return &icon_snow_showers_snow;
    case 77: return &icon_flurries;
    case 80: case 81: return is_day ? &icon_scattered_showers_day : &icon_scattered_showers_night;
    case 82: return &icon_heavy_rain;
    case 86: return &icon_heavy_snow;
    case 95: return is_day ? &icon_isolated_scattered_tstorms_day : &icon_isolated_scattered_tstorms_night;
    case 96: case 99: return &icon_strong_tstorms;
    default: return is_day ? &icon_mostly_cloudy_day : &icon_mostly_cloudy_night;
  }
}
